...
function mergeObjects(objects) {
  const a = Array.from(Object.entries(objects)).map((item) => item[1]);
...
function compareObjects(obj1, obj2) {
  ...
  const p = keys1.map((key) => obj1[key] === obj2[key]);
  return p.every((i) => i === true);
}
...
function makeWord(lettersObject) {
  const word = [];
  const a = Object.entries(lettersObject);
  a.forEach(([key, value]) => {
    value.forEach((item) => {
      word[item] = key;
    });
...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    const x = queue[i];
    if (x === 50) {
      if (cass[25] > 0) cass[25] -= 1;
      else return false;
    }
    if (x === 100) {
      if (cass[50] > 0 && cass[25] > 0) {
        cass[50] -= 1;
        cass[25] -= 1;
      } else if (cass[25] > 2) cass[25] -= 3;
      else return false;
    }
    cass[x] += 1;
  }
...
function fromJSON(proto, json) {
  const arg = Object.values(JSON.parse(json));
  const p = new proto.constructor(...arg);
  return p;
}
...
function group(array, keySelector, valueSelector) {
  ...
  const cities = countries.map((country) =>
    array.filter((item) => item[keys[0]] === country).map(valueSelector)
  );
  const entries = countries.map((item, index) => {
    return [item, cities[index]];
  });
